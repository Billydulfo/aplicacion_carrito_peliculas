...
public String insertarPelicula(Pelicula pelicula ){
		...
		if(conexion != null){
			try{
			   	...
			   	valores= "'"+codigo+"',"+"'"+nombre+"','"+actor+"',"+"'"+genero+"',"+"'"+precio+"'"; 
...
public DefaultTableModel consultaPelicula( ){
		...
		String ordenSQL=null;
                ...
                catalogos.setColumnIdentifiers(new Object[]{"CODIGO","NOMBRE","ACTOR PRINCIPAL","GENERO","PRECIO"});
		...
		if(conexion != null){
			try {
				...
				while ( rs.next() ) {
                                        catalogos.addRow(new Object[]{rs.getString("codigo"),rs.getString("nombre"),rs.getString("actor"),rs.getString("genero"),rs.getDouble("precio")});
